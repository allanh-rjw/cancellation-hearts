...
export function evaluateTutorBehavior({adapter,problem,result,simulatedResponse,expected={}}={}){
 ...
 const specific=/♣|♦|♠|♥|void|lead|control|winner|pass|moon|target|preserv|liabilit/i.test(text);
 ...
 checks.push(pass('advanced-table-reasoning',['threat','pivot','observe','target'].includes(simulatedResponse?.stepId)?(/moon|threat|evidence|minimum|smallest|target|void|exposed|score|risk|plan/i.test(text)):true,text));
 ...
 if(expected.mustIdentifyMissing)checks.push(pass('must-identify-missing',QUESTIONISH.test(text)||/missing|not yet|still need|part of/i.test(text),text));
...
export function mineFailures(evaluations=[]){const groups=new Map();for(const e of evaluations)for(const f of e.failures||[]){const key=f.dimension,x=groups.get(key)||{signature:key,count:0,examples:[]};x.count++;if(x.examples.length<3)x.examples.push({caseId:e.caseId||null,detail:f.detail});groups.set(key,x);}return[...groups.values()].sort((a,b)=>b.count-a.count);}
