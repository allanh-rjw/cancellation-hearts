import {runRegressionCorpus,runPassingPacket,runAdvancedPacket,runAlternativePathwayPacket,runBatch,runDiagnosticForPersona,runInstructionalJourney} from './harness.mjs';
...
else if(mode==='passing')result=await runPassingPacket({seed,throwOnFailure:true});
else if(mode==='advanced')result=await runAdvancedPacket({seed,throwOnFailure:true});
else if(mode==='alternative')result=await runAlternativePathwayPacket({seed});
...
if(mode==='alternative'&&result.validAlternativeRejectionRate>0)process.exitCode=2;
if(write&&result.kind==='batch')console.log(JSON.stringify(writeReports(result),null,2));
const summary=result.kind==='batch'?{kind:result.kind,learners:result.learners,seed:result.seed,placement:result.placement,evaluationMetrics:result.evaluationMetrics,failures:result.failures,validAlternativeRejectionRate:result.validAlternativeRejectionRate,isolation:result.isolation,packets:result.packets}:result;
