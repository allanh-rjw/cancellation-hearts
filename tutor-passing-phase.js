(function(){
  ...
  async function start({core,exercise,onComplete}){
    ...
    function renderPre(lastResult=null){
      body.innerHTML=`<div class="tutor-card passing-phase"><div class="eyebrow">Developing skill: passing</div><h3>Plan before the pass</h3><p>You will pass <strong>three cards ${esc(direction)}</strong>. Before seeing what comes back, decide what you want the hand to become.</p>${cardsMarkup(originalHand,selected,true)}<div class="passing-count">Selected: <strong>${selected.size}/3</strong></div><label class="passing-label" for="prePassPlan">Your pathway before the pass</label><textarea id="prePassPlan" rows="5" placeholder="What is your hand-level objective? Which three cards are you passing, why those three, and what do you hope the hand can do afterward?"></textarea>${feedbackMarkup(lastResult)}<div class="tutor-actions"><button id="submitPrePass">Evaluate pass plan</button></div></div>`;
      body.querySelectorAll('[data-pass-index]').forEach(btn=>btn.onclick=()=>{const i=Number(btn.dataset.passIndex);if(selected.has(i))selected.delete(i);else if(selected.size<3)selected.add(i);renderPre(lastResult);});
      document.getElementById('submitPrePass').onclick=async()=>{
        const plan=document.getElementById('prePassPlan').value.trim();if(selected.size!==3||!plan){renderPre({feedback:selected.size!==3?'Choose exactly three cards before submitting the pass plan.':'Explain the pathway you want the pass to create.'});return;}
        const codes=[...selected].sort((a,b)=>a-b).map(i=>originalHand[i]);
        const response={text:`${plan}\nPass cards: ${codes.join(', ')}`};
        const result=await core.evaluate({id:'prepass_pathway'},response,{exercise:{...exercise,hand:originalHand},answers:{passCards:codes,prePassPlan:plan}});
        if((result.score??0)<0.78){renderPre(result);document.getElementById('prePassPlan').value=plan;return;}
        const changed=handAfterPass(originalHand,[...selected]);
        renderPost(changed,plan,null);
      };
    }
    function renderPost(changed,prePlan,lastResult=null){
      ...
      document.getElementById('submitPostPass').onclick=async()=>{
        ...
        const result=await core.evaluate({id:'postpass_pathway'},{text:plan},{exercise:postExercise,answers:{prePassPlan:prePlan,passCards:changed.passed,receivedCards:changed.incoming,postPassPlan:plan}});
        if((result.score??0)<0.78){renderPost(changed,prePlan,result);document.getElementById('postPassPlan').value=plan;return;}
        postExercise.passing={direction,prePlan,postPlan:plan,passed:[...changed.passed],received:[...changed.incoming]};
