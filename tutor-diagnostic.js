(function(){
  ...
  function scorePlacement(evidence,assessmentResult){
    const scores=evidence.map(e=>Number(e.score?.value)||0),mean=scores.reduce((a,b)=>a+b,0)/Math.max(1,scores.length);
    const transferScores=evidence.slice(-2).map(e=>Number(e.score?.value)||0),transferMean=transferScores.reduce((a,b)=>a+b,0)/Math.max(1,transferScores.length);
    const independent=evidence.filter(e=>e.eligibility?.learnerModel&&e.administration?.independent),indMean=independent.reduce((s,e)=>s+(Number(e.score?.value)||0),0)/Math.max(1,independent.length);
    ...
    return {version:2,assessmentCoreSchemaVersion:window.CancellationHeartsAssessmentCore?.schemaVersion||null,source:'adaptive-trainer-assessment-core',blueprintId:BLUEPRINT_ID,completed:true,completedAt:new Date().toISOString(),ratings,total,maximum:16,level,label:LABELS[level],handScores:scores.map(x=>Math.round(x*100)),responses:evidence.length,assessmentResult,evidence:evidence.map(e=>({evidenceId:e.evidenceId,itemVersionId:e.item?.itemVersionId,constructIds:e.constructIds,score:e.score,eligibility:e.eligibility,administration:e.administration,timing:e.timing}))};
  }
  ...
  async function start({core,beginExercise}){
    const body=document.getElementById('tutorBody'),assessment=window.CancellationHeartsAssessmentCore;if(!body)return;if(!assessment)throw new Error('Adaptive Trainer Assessment Core is not loaded');
    ...
    const items=built.selection.selected.map(x=>x.item);let session=built.session,handIndex=0,phaseIndex=0,responses=[],evidence=[],passedByHand={},selected=new Set(),draft='',phaseResponses=[];
    core.state.diagnostic={completed:false,source:'adaptive-trainer-assessment-core',blueprintId:BLUEPRINT_ID,sessionId:session.sessionId,startedAt:session.startedAtUtc,handIndex:0};persist(core);
    ...
    async function submit(){
      ...
      session=recorded.session;responses.push(recorded.response);evidence.push(recorded.evidence);phaseResponses=[];handIndex++;phaseIndex=0;
      core.state.diagnostic={completed:false,source:'adaptive-trainer-assessment-core',blueprintId:BLUEPRINT_ID,sessionId:session.sessionId,startedAt:session.startedAtUtc,handIndex,evidenceCount:evidence.length};persist(core);
      if(handIndex>=items.length)return finish();render();
    }
    function finish(){
      const assessmentResult=assessment.result({session,evidence}),summary=scorePlacement(evidence,assessmentResult);core.state.diagnostic=summary;core.state.selfLevel=summary.level;core.profile=core.state;persist(core);
      body.innerHTML=`<div class="tutor-card diagnostic-results"><div class="eyebrow">Diagnostic complete</div><h3>Starting level: ${esc(summary.label)}</h3><p>The five hands were administered by the shared Adaptive Trainer Assessment Core. Placement translates its independent assessment evidence into the student-facing development level.</p><div class="diagnostic-dimensions">${Object.entries(summary.ratings).map(([k,v])=>`<div><span>${esc(k)}</span><strong>${v}/4</strong></div>`).join('')}</div><div class="framework-callout"><strong>Overall:</strong> ${summary.total}/16. This is a starting placement, not a permanent label. Ongoing independent and transfer evidence can move the level in either direction.</div><div class="tutor-actions"><button id="beginPlacedTutor">Begin at ${esc(summary.label)}</button></div></div>`;
