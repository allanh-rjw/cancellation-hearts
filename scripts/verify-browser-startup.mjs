...
async function inspectMode(port,baseUrl,mode){
  ...
  console.log(`${mode}: post-start OK (${started.phase}; causal=${started.causal}; tutor=${started.tutor}; deferred=${started.deferred})`);
  ws.close();
}
const server=createServer((req,res)=>{try{const requested=new URL(req.url,'http://localhost').pathname;if(requested==='/causal-loader.js'){const original=readFileSync(join(root,'causal-loader.js'),'utf8');const reentry="if(typeof renderCoach==='function' && typeof state!=='undefined' && state.players?.length) renderCoach();";if(!original.includes(reentry))throw new Error('causal re-entry signature changed');const code=original.replace(reentry,"window.__causalPlannerRefreshDeferred=true;");res.writeHead(200,{'content-type':'text/javascript; charset=utf-8','cache-control':'no-store'});res.end(code);return;}if(requested.startsWith('/v1/domains/')){res.writeHead(403,{'content-type':'application/json','cache-control':'no-store'});res.end(JSON.stringify({code:'authorization-denied'}));return;}const rel=requested==='/'?'index.html':decodeURIComponent(requested.slice(1));const clean=normalize(rel).replace(/^(\.\.(\/|\\|$))+/, '');const file=join(root,clean);if(!statSync(file).isFile())throw new Error('not file');res.writeHead(200,{'content-type':mime.get(extname(file))??'application/octet-stream','cache-control':'no-store'});res.end(readFileSync(file));}catch{res.writeHead(404);res.end('not found');}});
...
try{const debugPort=await waitForDebugPort(profile,chrome,()=>chromeError);const baseUrl=`http://127.0.0.1:${appPort}`;for(const mode of ['legacy','parity'])await inspectMode(debugPort,baseUrl,mode);console.log('browser-startup isolation: legacy + parity Start New Game OK with causal late render deferred');}finally{chrome.kill('SIGTERM');if(chrome.exitCode===null)await once(chrome,'exit');server.close();}
