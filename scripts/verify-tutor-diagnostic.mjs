...
let session=built.session;const responses=[],evidence=[];
const answers={
...
for(const item of selected){session=assessment.markItemPresented({session,item});const recorded=assessment.recordResponse({session,item,rawResponse:{phaseResponses:answers[item.id],reasoning:answers[item.id].map(x=>x.text).join('\n')},priorResponses:responses,supportBeforeResponse:'none',assistanceKinds:[],firstEncounter:true,context:'opening-diagnostic'});session=recorded.session;responses.push(recorded.response);evidence.push(recorded.evidence);assert.equal(recorded.evidence.administration.independent,true);assert.equal(recorded.evidence.eligibility.learnerModel,true);}
...
const placement=diagnostic.scorePlacement(evidence,result);
assert.equal(placement.source,'adaptive-trainer-assessment-core');
assert.equal(placement.responses,5);
assert.ok(['beginner','developing','advanced','expert'].includes(placement.level));
console.log('Tutor diagnostic uses the shared Adaptive Trainer Assessment Core.');
