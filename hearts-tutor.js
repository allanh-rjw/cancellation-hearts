(function(){
  ...
  function startTutor(){
    ...
    document.documentElement.classList.add('tutor-mode-active');
    window.scrollTo(0,0);
  ...
  function renderExpert(expert){
    const suits=expert.suitPlans?`<h3>How the expert reads each suit</h3><div class="expert-suit-grid">${Object.entries(expert.suitPlans).map(([s,v])=>`<div class="expert-suit"><h4>${esc(s)}</h4><p>${esc(v)}</p></div>`).join('')}</div>`:'';
    const controls=expert.controlBranches?.length?`<h3>What taking the lead is actually for</h3><div class="control-branches">${expert.controlBranches.map(b=>`<div class="control-branch"><h4>${esc(b.name||b.condition||'Control line')}</h4><p><strong>Use this line when:</strong> ${esc(b.condition)}</p><p><strong>Take control with:</strong> ${esc(b.entry)}</p><p><strong>Then lead:</strong> ${esc(b.followUp)}</p><p><strong>You are trying to induce:</strong> ${esc(b.hopedResponse)}</p><p><strong>The new state you want:</strong> ${esc(b.resultingState)}</p><p><strong>Preserve:</strong> ${esc(b.preserve)}</p></div>`).join('')}</div>`:'';
    const roles=expert.cardRoles?.length?`<h3>Plan for every card</h3><div class="role-table-wrap"><table class="role-table"><thead><tr><th>Card</th><th>Current role</th><th>Plan</th><th>Why</th><th>Role changes when…</th></tr></thead><tbody>${expert.cardRoles.map(r=>`<tr><td><strong>${esc(r.card)}</strong></td><td>${esc(r.role)}</td><td>${esc(r.action)}</td><td>${esc(r.why)}</td><td>${esc(r.changes)}</td></tr>`).join('')}</tbody></table></div>`:'';
    return `<h3>Expert read</h3><p>${esc(expert.read)}</p><p><strong>Strategy:</strong> ${esc(expert.strategy)}</p>${expert.desired?`<p><strong>Desired position:</strong> ${esc(expert.desired)}</p>`:''}${suits}${controls}<h3>Whole-hand pathway</h3><ol>${expert.pathway.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>${roles}`;
  }
  function renderExerciseIntro(){
    ...
    body.innerHTML=`<div class="tutor-card hand-card"><div class="eyebrow">${exercise.source==='curated'?'Curated teaching hand':'Transfer hand'}</div><h3>${esc(exercise.title)}</h3><div class="tutor-hand">${exercise.hand.map(c=>`<span class="mini-card ${/[DH]$/.test(c)?'red':''}">${esc(window.CancellationHeartsTutorAdapter.cardLabel(c))}</span>`).join('')}</div></div><div class="tutor-card worked-example-card"><h3>${show?'Worked example':'Your turn first'}</h3>${show?renderExpert(expert):`<p>Build the pathway before seeing the expert model. The tutor will give feedback one link at a time.</p>`}<button id="beginPathway" class="primary">${show?'Practice this pathway':'Build my pathway'}</button></div>`;
  ...
  function renderReview(){const expert=window.CancellationHeartsTutorAdapter.expertModel(exercise);const body=document.getElementById('tutorBody');const weak=core.masterySummary().slice(0,4);body.innerHTML=`<div class="tutor-grid"><div class="tutor-card"><h3>Your pathway</h3>${renderPathwaySoFar()}<h3>Expert comparison</h3>${renderExpert(expert)}</div><div class="tutor-card"><h3>Adaptive profile</h3><p>The tutor will use these estimates to choose how much scaffolding and what kind of next hand you receive.</p>${weak.map(x=>`<div class="mastery-row"><span>${esc(x.skill.replaceAll('_',' '))}</span><strong>${Math.round(x.value*100)}%</strong></div>`).join('')}<button id="nextTutorHand" class="primary">Next hand</button></div></div>`;document.getElementById('nextTutorHand').onclick=beginExercise;}
