(function(){
  ...
  function beginExercise(){exercise=core.selectExercise();stepIndex=0;answers={};renderExerciseIntro();window.scrollTo({top:0,behavior:'instant'});}
  function renderExpert(expert){
    const suitData=expert.suitPlan||expert.suitPlans||[];
    const controlData=expert.controlPlans||expert.controlBranches||[];
    const cardData=expert.cardPlan||expert.cardRoles||[];
    const desired=expert.desiredState||expert.desired||'';
    const suits=suitData.length?`<div class="worked-section"><h3>How the expert reads each suit</h3><div class="suit-plan-grid">${suitData.map(v=>`<div class="worked-block"><h4>${esc(v.suit||v.name||'Suit')}</h4><p>${esc(v.plan||v.text||v)}</p></div>`).join('')}</div></div>`:'';
    const controls=controlData.length?`<div class="worked-section"><h3>What taking the lead is actually for</h3><p class="small-copy">Every control line must specify a condition, an entry, a follow-up lead, the response it is trying to induce, and the resulting state.</p><div class="control-plan-list">${controlData.map((b,i)=>`<div class="control-plan"><div class="control-plan-title">Control line ${i+1}: ${esc(b.entry||'Entry')}</div><p><strong>Use this line when:</strong> ${esc(b.condition)}</p><p><strong>Take control with:</strong> ${esc(b.entry)}</p><p><strong>Then lead:</strong> ${esc(b.followUp)}</p><p><strong>You are trying to induce:</strong> ${esc(b.induce||b.hopedResponse)}</p><p><strong>The new state you want:</strong> ${esc(b.creates||b.resultingState)}</p><p><strong>Preserve:</strong> ${esc(b.preserve)}</p></div>`).join('')}</div></div>`:'';
    const roles=cardData.length?`<div class="worked-section"><h3>Plan for every card in the hand</h3><p class="small-copy">The role is current, not permanent. The expert keeps reassessing as cards disappear and cancellations change effective rank.</p><div class="card-plan-table"><div class="card-plan-head"><span>Card</span><span>Current role</span><span>Plan</span><span>Why</span><span>Role changes when…</span></div>${cardData.map(r=>`<div class="card-plan-row"><strong class="role-card">${esc(window.CancellationHeartsTutorAdapter.cardLabel(r.card||''))}</strong><span>${esc(r.role)}</span><span>${esc(r.action)}</span><span>${esc(r.why)}</span><span>${esc(r.changes)}</span></div>`).join('')}</div></div>`:'';
    return `<div class="worked-example"><div class="worked-section"><h3>Expert read</h3><p>${esc(expert.read)}</p><p><strong>Strategy:</strong> ${esc(expert.strategy)}</p>${desired?`<p><strong>Desired position:</strong> ${esc(desired)}</p>`:''}</div>${suits}${roles}${controls}<div class="worked-section"><h3>Whole-hand pathway</h3><ol>${expert.pathway.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div></div>`;
  }
  ...
  function renderReview(){const expert=window.CancellationHeartsTutorAdapter.expertModel(exercise);const body=document.getElementById('tutorBody');const weak=core.masterySummary().slice(0,4);body.innerHTML=`${handStrip(true)}<div class="tutor-grid"><div class="tutor-card"><h3>Your pathway</h3>${renderPathwaySoFar()}<h3>Expert comparison</h3>${renderExpert(expert)}</div><div class="tutor-card"><h3>Adaptive profile</h3><p>The tutor will use these estimates to choose how much scaffolding and what kind of next hand you receive.</p>${weak.map(x=>`<div class="mastery-row"><span>${esc(x.skill.replaceAll('_',' '))}</span><strong>${Math.round(x.value*100)}%</strong></div>`).join('')}<button id="nextTutorHand" class="primary">Next hand</button></div></div>`;document.getElementById('nextTutorHand').onclick=beginExercise;}
