(function(){
  ...
  function diagnosis(id,t){
    ...
    if(id==='pivot'){
      const minimal=/minimum|smallest|one heart|only enough|without.*abandon|return.*plan|return.*path|preserv|break.*(?:moon|threat).*(?:return|resume)|(?:return|resume).*(?:after|once).*break/.test(t),overreact=/every remaining trick|take every|win everything|abandon.*whole|abandon.*entire/.test(t);
